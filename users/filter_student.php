  ...
  .fakeimg {
      ...
      <div id="body-bg">
         ...
         <div class="container bottom-border">
            <div class="row padding-top-40">
				...
				<div class="col-md-3" style="heidh:auto; background:#f4f4f4; border: 2px solid #cccccc; margin-left:10px; margin-top:10px; margin-bottom:14px; padding-bottom:10px;">
	...
	{ $query_all='';
		...
		//Область проживання	
						...
						if($select_region=="")$query_sr="";
						else $query_sr="AND s_region IN ('".$select_region."')";	 
							 	
		//Діти сироти
						
						$check_sirot=$_POST ["check_sirot"];
						if($check_sirot=="")$query_ds="";
						else $query_ds="AND s_sirota IN ('Так')";	
	
		//Переселенці
						$check_peres=$_POST ["check_peres"];
						if($check_peres!="Так")$query_per="";
						else $query_per="AND s_peresel IN ('Так')";
				
								
		//Інваліди
						$check_ivalid=$_POST ["check_ivalid"];
						if($check_ivalid!="Так")$query_ivalid="";
						else $query_ivalid="AND s_inval IN ('Так')";
						   
		//Малозабезпечені
						$check_malozab=$_POST ["check_malozab"];
						if($check_malozab!="Так")$query_malozab="";
						else $query_malozab="AND s_malozab IN ('Так')";
						  
								   
		//Учасники бойвих дій та їх діти				  
						$check_uchbd=$_POST ["check_uchbd"];
						if($check_uchbd!="Так")$query_uchbd="";
						else $query_uchbd="AND s_war_act IN ('Так')";

							   
	echo $query_all=$query_vz.$query_gz.$query_sp.$query_sk.$query_vg.$query_st.$query_os.$query_vik.$query_yer.$query_mp.$query_sr.$query_ds.$query_per.$query_ivalid.$query_malozab;
						 ...
						 echo '
								<div class="panel panel-default style:margin-top:0px;font-size:10px;">
									...
									<table style="table-layout:fixed; width:720px;">  
										...
										<td style="width:280px;  text-decoration-line: underline; padding-left:10px"><strong><a href="view_student.php?id_st='.$row["s_id"].'">Перегляд</a>|
	...
	else {$result = mysqli_query($linc, "SELECT * FROM student");
