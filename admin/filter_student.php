			...
			try {
				$insert_query = "INSERT INTO old_student (
							...
							SELECT 
								...
								'$operation_date', '$operation_name'
							FROM student
							WHERE s_id = '$id_st'"; // ваш SELECT ... INSERT INTO old_student
...
.dropdown-filter-item {
  padding: 5px 0;
	...
	{ $query_all='';
		...
		if (empty($selectedGz)) {
			$allGz = array();
			$gz_query_all = "SELECT DISTINCT s_galuz AS gz FROM student ORDER BY s_galuz ASC";
	...
	$query_uchbd . $query_ditzag . $query_stepver . $query_shaht . $query_group . $query_vik . $query_region. $query_social. $order_clause;

	 $query_all;
	session_start();
	$_SESSION['query_all'] = $query_all;

		$result_to_xlsx = "SELECT * FROM student WHERE $query_all";
...
$query_count = "SELECT COUNT(*) as total FROM student WHERE $query_all";
...
$query_with_limit = "SELECT * FROM student WHERE $query_all LIMIT $offset, $limit";
...
if (mysqli_num_rows($result_searh) == 0) {
    ...
    while ($row = mysqli_fetch_array($result_searh)) {
        echo '<tr>
                ...
                <td class="text-center" style="text-decoration-line: underline; padding-left:5px; width: 500px;">
                    <strong>
                        <a href="view_student.php?id_st=' . $row["s_id"] . '">Перегляд</a> 
                            ...
                            if (in_array($_SESSION['auth_user'], ['admin', 'editor'])) {
								...
								<a href="edit_student.php?id_st=' . $row["s_id"] . '">Змінити</a> |
								                        <a  href="filter_student.php?id_st=' . $row["s_id"] . '&pr_st=' . $row["s_pr"] . '&im_st=' . $row["s_im"] . '&bat_st=' . $row["s_bat"] . '" onclick="return confirmSpelll();">Видалити</a>
