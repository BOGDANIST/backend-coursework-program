   ...
   if (isset($_POST['submit_add_group']))
    {
		...
		$result=mysqli_query($linc, "SELECT * FROM st_group");
        $row = mysqli_fetch_array($result);
			do{
			  if ($_POST["form_im_group"]==$row['g_im'])
			  {
		...
		if($error==""){
      	mysqli_query($linc, "INSERT INTO st_group SET
					g_im='".$_POST["form_im_group"]."',
					g_galuz='".$_POST["g_gz"]."',
					g_spec='".$_POST["g_sp"]."',
					g_specz='".$_POST["g_sz"]."',
					g_course='".$_POST["g_kurs"]."',
					g_vipusc='".$_POST["g_vip"]."',
					g_count_stud='".$_POST["g_count_stud"]."',
					g_count_derg='".$_POST["g_count_rz"]."',
					g_count_comerc='".$_POST["g_count_contr"]."',
					g_nastav='".$_POST["g_nast"]."',
					g_formnavch='".$_POST["g_fn"]."'
					");
					$success = "Група успішно додана!";
		}
	};
                        ...
                        <div class="col-md-8 col-md-offset-2 col-sm-offset-2">
							...
							<form class="signup-page" method="POST">
								...
								$gz_result = mysqli_query($linc, $gz_query);
									...
									while ($row_gz = mysqli_fetch_assoc($gz_result)) {
										$value = htmlspecialchars($row_gz['gz']);
											$im_galuz = htmlspecialchars($row_im_galuz['g_gz']);
											// Якщо ця спеціальність була вибрана, додаємо checked
											$isChecked = in_array($im_galuz, $selected) ? 'checked' : '';
									...
									if ($spec_result) {
										while ($row_spec = mysqli_fetch_assoc($spec_result)) {
											...
											// Якщо ця Спеціалізації була вибрана, додаємо checked
									...
									if ($specializ_result) {
										while ($row_specializ = mysqli_fetch_assoc($specializ_result)) {
											...
											// Якщо ця спеціальність була вибрана, додаємо checked
											$isChecked = in_array($specializ, $selected) ? 'checked' : '';
											echo '
								...
								<label for="sel8">Наставник групи</label>
								 <input type="text" class="form-control" id="sel8"   name="g_nast" style="background:white; color:#0c0e0c; border:none;"   name="form_bat_stud">
								
								<p>
								 <input type="submit" class=" form-control bg-success" id="submit-form" name="submit_add_group" value="Додати" style="color:#0c0e0c; border: 2px solid #c1aaaa; margin-top:25px; color:white;">
								</p>
															<p>
